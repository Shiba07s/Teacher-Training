...
<body>
	<form id="updateForm" action="updateTraining" method="post">
		...
		<div class="row">
			<div class="form-group col-md-3">
				<label for="preferdlocation">Venue Location:</label>
				<div class="form-floating">
	...
	<script>
		...
		function updateTraining() {
			...
			$.ajax({
				type : "PUT",
				url : "${pageContext.request.contextPath}/api/edit/" + $('[name=refplannerid]').val(),
		...
		function editCalender() {
			...
			if (calenderId) {
				$
						.ajax({
							type : "GET",
							url : "${pageContext.request.contextPath}/api/getdatabyid/" + calenderId, // Replace with your actual endpoint
							dataType : "json",
							success : function(data) {
								...
								$('#tsubject').val(data[0].tsubject);
								$('#tcategory').val(data[0].tcategory);
								$('#tmode').val(data[0].tmode);
								$('#ttargetgroup').val(data[0].ttargetgroup);
								$('#tmonth').val(data[0].tmonth);
								$('#tyear').val(data[0].tyear);
								$('#tname').val(data[0].tname);
								$('#tdescription').val(data[0].tdescription);
								$('#tgrade').val(data[0].tgrade);
								$('#tagency').val(data[0].tagency);
								$('#tspell').val(data[0].tspell);
								$('#preferdlocation').val(data[0].preferdlocation);
								$('#numberofstakeholder').val(
										data[0].numberofstakeholder);
								$('#numberdayneeded').val(data[0].numberdayneeded);
								$('#thoursperday').val(data[0].thoursperday);
								$('#totalhours').val(data[0].totalhours);
								$('#trainingregstartdt').val(
										data[0].trainingregstartdt);
								$('#trainingregenddt').val(
										data[0].trainingregenddt);
								$('#tstatus').val(data[0].tstatus);
