...
<body>
	...
	<a href="${pageContext.request.contextPath}/AddNewStatus.jsp" class="btn btn-success mt-2">Add New
		Status +</a>
	...
	<script>
		...
		function fetchDataAndDisplay() {
			$.ajax({
				type : "GET",
				url : "${pageContext.request.contextPath}/api/status",
				dataType : "json",
		...
		function deleteTraining(button, statuscode) {
			...
			if (isConfirmed) {
				$.ajax({
					type : "Delete",
					url : "${pageContext.request.contextPath}/api/deleteStatus/" + statuscode,
		...
		function editTraining(statuscode) {
			...
			if (isConfirmed) {
				...
				var editUrl = "${pageContext.request.contextPath}/EditTrainingStatus.jsp?statuscode=" + statuscode;
