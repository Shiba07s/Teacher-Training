...
<body>
	...
	<a href="${pageContext.request.contextPath}/AddNewResource.jsp" class="btn btn-success mt-2">Add
		New ResourceType +</a>
	...
	<script>
		...
		function deleteTraining(button, resourcecode) {
			...
			if (isConfirmed) {
				...
				$
						.ajax({
							type : "Delete",
							url : "/api/deleteResourceType/" + resourcecode,
							success : function(response) {
								var table = $('#trainingTable').DataTable();
								table.row(row).remove().draw(false);
								toastr
										.success("ResourceType deleted successfully");
								// Reload or update the data after deletion
								fetchDataAndDisplay();
							},
							error : function(xhr, status, error) {
								toastr.error("Failed to delete ResourceType: "
										+ error);
							}
						});
			} else
		...
		function editTraining(resourcecode) {
			...
			if (isConfirmed) {
				...
				var editUrl = "EditResourceType.jsp?resourcecode="
						+ resourcecode;
