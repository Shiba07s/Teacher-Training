...
<body>
	...
	<a href="${pageContext.request.contextPath}/Add New Spell.jsp" class="btn btn-success mt-2">Add
		New Spell +</a>
	...
	<script>
		...
		function displayTrainingData(data) {
			...
			for (var i = 0; i < trainings.length; i++) {
				...
				row.append("<td >" + training.spellcode + "</td>");

				row.append("<td>" + training.spell + "</td>");
		...
		function deleteTraining(button, spellcode) {
			...
			if (isConfirmed) {
				$.ajax({
					type : "Delete",
					url : "${pageContext.request.contextPath}/api/deleteSpell/" + spellcode,
		...
		function editTraining(spellcode) {
			...
			if (isConfirmed) {
				...
				var editUrl = "${pageContext.request.contextPath}/EditTrainingSpell.jsp?spellcode=" + spellcode;
